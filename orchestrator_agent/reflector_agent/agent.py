...
report_interpretation_skill = load_skill_from_dir(
    f"{current_dir}/skills/report-interpretation-skill"
)
thinking_pattern_skill = load_skill_from_dir(
    f"{current_dir}/skills/thinking-pattern-skill"
)
# webapp_pt_skill = load_skill_from_dir(
#     f"{current_dir}/skills/webapp_pt_skill"
# )

tool_filter=["get_session_context","log_reasoning","distill_knowledge",
"score_branches","retrieve_knowledge","add_finding","get_intent_incidents"]
...
skills  = SkillToolset(
                    skills=[thinking_pattern_skill,report_interpretation_skill]
)
reflector_agent = Agent(
    ...
    description="""You are the Reflector agent for RedTeam V9 autonomous penetration testing.
...
After reviewing findings, immediately signal CONTINUE, PIVOT, or COMPLETE to the
Orchestrator. Never ask the user for guidance. Make the decision yourself based on
finding count and branch confidence.
    """,
)
